...
@task
def handle_bounce(message, address=None, host=None):
    logger.debug('Got Bounce address=%s host=%s', address, host)
    ...
    try:
        ...
        logger.error('Could not parse Bounce as ARF: %s', e)
        return
    ...
    if not list_index or not m:
        logger.error("Bounce report original message does not contain a valid List-Id header: %s", list_index)
        return
    ...
    logger.info("Got Bounce for Campaign %s: %s", campaign, r.email)
    r.append_log(bounce=True, blocked=True, msg="Bounce report")
